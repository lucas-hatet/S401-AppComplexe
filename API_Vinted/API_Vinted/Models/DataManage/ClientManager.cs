...
namespace API_Vinted.Models.DataManage
{
    public class ClientManager : IDataRepository<Client>
    {
        ...
        {
            return await _context.Clients.Include(c => c.AdresseFacturation.Ville).Include(c => c.AvisSur).ThenInclude(a => a.Acheteur).Include(c => c.Articles).ThenInclude(a => a.Marque).Include(c => c.Photo).FirstOrDefaultAsync(a => a.IDClient == id);
